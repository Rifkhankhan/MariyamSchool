...
const Dashboard = () => {
	...
	const mycommentedBlogs = blogs.filter(blog =>
		blog.comments.includes(
			blog.comments.some(comment => comment.user.id === user?._id)
		)
	)
	...
	return (
		<div className={styles.Dashboard}>
			...
			{myBlogs.length > 0 ? (
				<DashboardTable blogs={myBlogs} title="My Blogs" />
			) : (
				<DashboardTable blogs={datas} title="My Blogs" />
			)}
			{myLikedBlogs.length > 0 ? (
				<DashboardTable blogs={myLikedBlogs} title="Blogs i liked" />
			) : (
				<DashboardTable blogs={datas} title="Blogs i liked" />
			)}
			{mycommentedBlogs.length > 0 ? (
				<DashboardTable blogs={mycommentedBlogs} title="Blogs i commented" />
			) : (
				<DashboardTable blogs={datas} title="Blogs i commented" />
			)}
		</div>
	)
}
